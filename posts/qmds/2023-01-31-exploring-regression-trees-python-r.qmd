...
Also, an interesting difference between the Python version and the R version is the metrics are not the same. The R version uses something called *Standard Deviation Reduction* which tries to condition the split on minimizing the Standard Deviation. But, the [Python documentations](https://scikit-learn.org/stable/modules/generated/sklearn.tree.DecisionTreeRegressor.html#sklearn.tree.DecisionTreeRegressor) doesn't even have that as a choice but defaults to squared error instead.